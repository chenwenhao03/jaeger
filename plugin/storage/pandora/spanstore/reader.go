...
//}
func NewSpanReader(client pipeline.PipelineAPI) spanstore.Reader {
	return newSpanReader(client)
}
func newSpanReader(client pipeline.PipelineAPI) *SpanReader {
	...
	return &SpanReader{
		...
		serviceOperationStorage: NewServiceOperationStorage(ctx, client,0), // the decorator takes care of metrics
	}
}
